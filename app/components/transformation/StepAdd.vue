...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  try {
    ...
    router.push(`/transformation/${props.transformation.id}`)
    ...
    <UForm :schema="schema" :state="state" class="space-y-6" @submit="onSubmit">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <UFormField label="Quantité utilisée" name="quantity" required>
          ...
          <template #hint>
            <span class="text-xs" :class="remainingAfter < 0 ? 'text-red-500 font-medium' : 'text-gray-500'">
              Stock disponible: {{ transformation.remaining_quantity.toFixed(2) }} kg
            </span>
          </template>
        </UFormField>
      </div>
      ...
      <div 
        ...
        :class="remainingAfter < 0 ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800' : 'bg-gray-50 dark:bg-gray-800'"
      >
        <div class="flex justify-between text-sm">
          <span class="text-gray-600 dark:text-gray-400">Quantité à retirer:</span>
          <span class="font-semibold text-gray-900 dark:text-white">{{ state.quantity }} kg</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-gray-600 dark:text-gray-400">Stock après opération:</span>
          <span 
            class="font-semibold"
            :class="remainingAfter < 0 ? 'text-red-600' : 'text-green-600'"
          >
            {{ remainingAfter.toFixed(2) }} kg
            <span v-if="remainingAfter < 0" class="text-xs ml-1">(Impossible)</span>
          </span>
        </div>
      </div>
