...
const expandableFields: ExpandableField[] = [
  ...
  { 
    key: 'price_per_unit', 
    label: 'Prix Unitaire',
    format: (val) => new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XOF' }).format(Number(val))
  },
  { 
    key: 'total_price', 
    label: 'Prix Total',
...
function getRowItems(row: Row<PurchaseItem>) {
  return [
    {
      type: 'label' as const,
      label: 'Actions'
    },
    {
      ...
      icon: 'i-lucide-eye',
      onSelect() {
        navigateTo(`/purchases/${row.original.id}`)
      }
    },
    {
      label: 'Modifier',
...
const columns: TableColumn<PurchaseItem>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XOF' }).format(Number(row.getValue('price_per_unit')))
  },
  {
    accessorKey: 'total_price',
    header: 'Total',
    meta: { class: { th: 'hidden md:table-cell text-right', td: 'hidden md:table-cell text-right font-semibold' } },
    cell: ({ row }) => new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XOF' }).format(Number(row.getValue('total_price')))
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h(UBadge, { label: getCategoryName(categoryId), style: getCategoryStyle(categoryId), size: 'sm' })
    }
  },
  {
    id: 'stock',
  ...
  },
  {
    ...
    cell: ({ row }) => h(UDropdownMenu, { content: { align: 'end' }, items: getRowItems(row) }, 
  ...
  <UCard>
    ...
    >
      <template #expanded="{ row }">
        <div class="p-4 grid grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-800/50 md:hidden">
          <div 
          ...
          >
            ...
            >
              {{ field.format 
                ? field.format(row.original[field.key as keyof PurchaseItem], row.original) 
